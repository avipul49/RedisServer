...
public class H264Player implements Runnable {
	...
	ConcurrentLinkedQueue<byte[]> bufferedFrames = new ConcurrentLinkedQueue<byte[]>();
	private static H264Player player;

	public static H264Player getPlayer() {
		if (player == null) {
	...
	public void play(byte[] args) {
		bufferedFrames.add(args);
	}
	...
	public boolean playFile(byte[] in) {
		H264Decoder codec;
		...
		try {
			...
			// avpkt must contain exactly 1 NAL Unit in order for decoder to
			// decode correctly.
			// thus we must read until we get next NAL header before sending it
			// to decoder.
			// Find 1st NAL

			inputData = bufferedFrames.poll();
			fin = new ByteArrayInputStream(inputData);
			...
			while (!(cacheRead[0] == 0x00 && cacheRead[1] == 0x00
					&& cacheRead[2] == 0x00 && cacheRead[3] == 0x01)) {
				...
				while (!bufferedFrames.isEmpty()) {
					...
					while (hasMoreNAL) {
						...
						if (cacheRead[4] == -1)
							hasMoreNAL = false;
						while (!(cacheRead[0] == 0x00 && cacheRead[1] == 0x00
								&& cacheRead[2] == 0x00 && cacheRead[3] == 0x01 && isEndOfFrame(cacheRead[4]))
								&& hasMoreNAL) {
						...
						// System.out.println(String.format("Offset 0x%X, packet size 0x%X, nal=0x%X",
						// fileOffset, dataPointer, inbuf_int[4] & 0x1F));
						...
						try {
							while (avpkt.size > 0) {
								len = c.avcodec_decode_video2(picture,
										got_picture, avpkt);
								if (len < 0) {
									System.out
											.println("Error while decoding frame "
													+ frame);
									// Discard current packet and proceed to
									// next packet
									break;
								} // if
								if (got_picture[0] != 0) {
									...
									displayPanel.lastFrame = displayPanel
							...
							// Any exception, we should try to proceed reading
							// next
							// packet!
							ie.printStackTrace();
						} // try
